...
(function() {
  ...
  pageFunc = {
    ...
    imgContain: function() {
      var i, item, len, para, results;
      if (document.getElementsByClassName("detail")[0] != null) {
        para = document.getElementsByClassName("post_body")[0].getElementsByTagName("p");
        results = [];
        for (i = 0, len = para.length; i < len; i++) {
          item = para[i];
          if ($(item).find("img").length) {
            results.push($(item).addClass("img_inside"));
          }
        }
        return results;
      }
  ...
  plugin = {
    ...
    fancybox: function() {
      if (document.getElementsByClassName("fancybox")[0] != null) {
